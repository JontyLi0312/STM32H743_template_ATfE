...
uint8_t Touch_IIC_WaitACK(void)
{
    ...
    if (HAL_GPIO_ReadPin(Touch_IIC_SDA_PORT, Touch_IIC_SDA_PIN) !=
        0) // 判断设备是否有做出响应
    {
        Touch_IIC_SCL(0);
        Touch_IIC_Delay(IIC_DelayVaule);
...
uint8_t Touch_IIC_ReadByte(uint8_t ACK_Mode)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        Touch_IIC_Delay(IIC_DelayVaule);
        IIC_Data |=
            (HAL_GPIO_ReadPin(Touch_IIC_SDA_PORT, Touch_IIC_SDA_PIN) & 0x01);
        Touch_IIC_SCL(0);
        Touch_IIC_Delay(IIC_DelayVaule);
    }
